...
- `cargo test -p ytdlp-core`：**151 个用例**（`#[cfg(test)]` 静态计数，分布：model 32、download 15、exec 13、probe 13、config 12、transcode 11、history 10、merge 8、tool_download 8、cookies 7、worker 6、cli 5、paths 4、timefmt 4、thumbs 3）。另有 `src-tauri`（login）4 个用例，`cargo test --workspace` 时一并执行（CI 门禁只跑 core）。
