...
- `cargo test -p ytdlp-core`：**148 个用例**（`#[cfg(test)]` 静态计数，分布：model 31、download 15、exec 13、probe 13、config 12、transcode 11、history 10、merge 8、tool_download 7、cookies 7、worker 6、cli 5、paths 4、timefmt 4、thumbs 2）。另有 `src-tauri`（login）4 个用例，`cargo test --workspace` 时一并执行（CI 门禁只跑 core）。
...
2. 依赖下载/更新：设置-依赖 对四个工具执行"下载"，确认落在 `tools\` 且路径被写回输入框；再点"更新"——yt-dlp / deno 应给出"已是最新（版本号），无需更新"或更新到新版本，ffmpeg / ffprobe 靠 `tools\installed.json` 指纹判断同样能提示"已是最新"；若把某个路径填成 PATH 里的工具（如 `C:\Windows\System32` 下），点"更新"应提示手动更新而不是去改它。下载中按钮显示"取消 <阶段> <百分比>"并持续前进（连接 → 下载 → 校验 → 解压 → 安装）。
